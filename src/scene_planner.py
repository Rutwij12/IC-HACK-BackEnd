...
class SceneEvaluation(BaseModel):
    ...
    passes_criteria: bool = Field(description="Whether scene plan meets all criteria")
    feedback: str = Field(description="Feedback if plan fails criteria, empty if passes")
...
class ScenePlanner:
    def __init__(self, scene_prompt: str, 
        ...
        self.evaluator = LLMWrapper(model=evaluator_model).with_structured_output(SceneEvaluation)
